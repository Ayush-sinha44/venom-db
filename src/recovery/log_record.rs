...
impl LogRecord {
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        ...
        match self {
            ...
            Self::Insert { lsn, txn_id, page_id, slot_id, data } => {
            ...
            Self::Delete { lsn, txn_id, page_id, slot_id, old_data } => {
            ...
            Self::Update { lsn, txn_id, page_id, slot_id, old_data, new_data } => {
    ...
    pub fn from_bytes(buf: &[u8]) -> Option<Self> {
        ...
        macro_rules! read_u16 {
            () => {{ let v = u16::from_le_bytes(buf[off..off+2].try_into().ok()?); off += 2; v }};
        }
        macro_rules! read_u32 {
            () => {{ let v = u32::from_le_bytes(buf[off..off+4].try_into().ok()?); off += 4; v }};
        }
        macro_rules! read_u64 {
            () => {{ let v = u64::from_le_bytes(buf[off..off+8].try_into().ok()?); off += 8; v }};
        }
        ...
        Some(match record_type {
            ...
            Self::TYPE_INSERT => {
                let lsn = read_u64!(); let txn_id = read_u32!();
                let page_id = read_u32!(); let slot_id = read_u16!();
                let data = read_bytes!();
                Self::Insert { lsn, txn_id, page_id, slot_id, data }
            }
            Self::TYPE_DELETE => {
